---
title: "Paper Sharing: [主动提示与主动提问：agent 在不确定性下猜测用户意图]"
...
- **Slow mode**：仅当 $|p_\text{accept}^F - \tau(p_\text{need}^F)| \leq \delta_\text{slow}$ 时（靠近决策边界，最不确定时）才触发深度推理
- 实验最优参数 $\delta=0.1$ 下，仅约 **11%** 的样本进入 Slow mode
...
- $-\mathbf{1}[y_\text{need}^{pred}=1](q_\text{accept}-y_\text{accept})^2$：在 teacher 判断"确实需要"的前提下，对"是否会接受"的概率估计也要准确
...
2. **$\mathbf{1}_\text{ask}$ 强制至少一次澄清**：早期训练中模型可能退化为"从不提问，直接给答案"（因为答对了也有 Output Reward）。$\mathbf{1}_\text{ask}$ 确保模型在正确答案的条件下还需要完成至少一次有效澄清才能获得 Reasoning Reward。这是训练初期防止模式坍缩的机制，不是永久约束。

---

3. **$E(r) \cdot H_\text{LLM}(r)$ 乘法**：效率和有效性不能单独优化。消融验证：
...
GPS 用 DAG 显式枚举所有条件路径，保证逻辑完备性。类比地，主动提示可以在介入之前，先对用户意图空间构建结构化表示——不是单一的 pneed 概率，而是多个候选意图及其条件关系，当且仅当当前观测能将意图空间收窄到单一分支时才介入，否则沉默。这可以直接解决 ProactiveBench 中 False-Alarm 过高的根本问题（模型在意图仍有歧义时过早介入）。

**PIR 的 PE 触发 → 主动提示的"何时保持沉默"判断**

PIR 通过预测熵识别推理链中的高不确定区域。这与 PRISM 的 pneed 估计本质类似，但 PIR 衡量的是模型内部对推理方向的不确定性，而 PRISM 衡量的是对用户需求的不确定性。两者可以结合：当模型对用户意图的内部推断熵高时，提高门控阈值，做出更保守的介入决策。
