...
- **Slow mode**：仅当 $|p_{\text{accept}}^F - \tau(p_{\text{need}}^F)| \leq \delta_{\text{slow}}$ 时（靠近决策边界，最不确定时）触发深度推理。
...
- $-\mathbf{1}[y_{\text{need}}^{(\text{pred})}=1](q_{\text{accept}} - y_{\text{accept}})^2$：在 teacher 判断"确实需要"的前提下，对"是否会接受"的概率估计也要准确
